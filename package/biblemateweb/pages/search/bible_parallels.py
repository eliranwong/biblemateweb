from biblemateweb import BIBLEMATEWEB_DATA, load_topic_vectors_from_db, get_translation
from biblemateweb.fx.bible import get_bible_content
...
def fetch_parallels_topic(path):
    db = os.path.join(BIBLEMATEWEB_DATA, "collections3.sqlite")
...
async def fetch_topic_matches_async(query):
    ...
    db_file = os.path.join(BIBLEMATEWEB_DATA, "vectors", "collection.db")
    sql_table = "PARALLEL"
...
def search_bible_parallels(gui=None, q='', **_):
    ...
    with ui.dialog() as dialog, ui.card().classes('w-full max-w-md'):
        ui.label("Bible Parallels...").classes('text-xl font-bold text-secondary mb-4')
