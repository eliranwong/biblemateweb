...
def fetch_bible_dictionaries_entry(path):
    db = os.path.join(BIBLEMATEWEB_DATA, "data", "dictionary.data")
...
async def fetch_bible_dictionaries_matches_async(query):
    ...
    db_file = os.path.join(BIBLEMATEWEB_DATA, "vectors", "dictionary.db")
...
def fetch_all_dictionaries():
    db = os.path.join(BIBLEMATEWEB_DATA, "vectors", "dictionary.db")
    with apsw.Connection(db) as connn:
        cursor = connn.cursor()
