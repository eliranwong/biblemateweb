...
from biblemateweb import config, BIBLEMATEWEB_APP_DIR, USER_DEFAULT_SETTINGS, getBibleVersionList, getLexiconList
from biblemateweb.pages.home import *
from biblemateweb.js.tooltip import TOOLTIP_JS
from biblemateweb.css.original import get_original_css
from biblemateweb.css.tooltip import get_tooltip_css
from biblemateweb.fx.tooltips import get_tooltip_data
        ...
        with ui.expansion('AI Backend', icon='memory').classes('w-full rounded-lg'):
            with ui.column().classes('w-full p-4 gap-4'):
                ui.select(label='AI Backend',
                          options=['default', 'googleai', 'openai', 'anthropic', 'cohere', 'deepseek', 'mistral', 'xai', 'azure_anthropic', 'azure_cohere', 'azure_deepseek', 'azure_mistral', 'azure_openai', 'azure_xai', 'ollamacloud', 'custom']) \
...
def main():
    ...
    ui.run(
        ...
        port=config.port,
        title='BibleMate AI',
        favicon=os.path.expanduser(config.avatar) if config.avatar else os.path.join(BIBLEMATEWEB_APP_DIR, 'eliranwong.jpg')
    )
