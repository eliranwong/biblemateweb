...
bible_audio = ["1.mp3", "2.mp3", "3.mp3", "4.mp3", "5.mp3", "6.mp3"]
bible_audio = [os.path.join("audio", file) for file in bible_audio]
        ...
        with ui.card().classes('w-full max-w-4xl mx-auto mt-8 p-6'):
            ...
            with ui.column().classes('w-full gap-2'):
                """for verse_num, verse_text in bible_chapter:
                    with ui.row().classes('w-full items-start gap-3 p-3 rounded hover:bg-gray-50'):
                        # Audio control button
                        btn = ui.button(icon='volume_off', 
                                       on_click=lambda v=verse_num: self.toggle_verse(v))
                        btn.classes('text-blue-600')
                        self.verse_buttons[verse_num] = btn
                        
                        # Verse number and text
                        with ui.column().classes('flex-grow'):
                            ui.label(f'Verse {verse_num}').classes('font-semibold text-sm text-gray-600')
                            ui.label(verse_text).classes('text-base leading-relaxed')"""
...
ui.run(title='Bible Audio Player', port=9998)