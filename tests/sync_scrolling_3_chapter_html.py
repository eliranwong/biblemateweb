    ...
    with ui.splitter(value=50).classes('w-full h-[80vh]') as splitter:
        ...
        with splitter.before:
            with ui.scroll_area().classes('w-full h-full scroll-area-1'):
                # Pass the single HTML blob
                # Add a 'chapter-1' class for our JS selector
                ui.html(chapter_html_1, sanitize=False).classes('p-4 chapter-1')

        # Right pane (e.g., NIV)
        with splitter.after:
            with ui.scroll_area().classes('w-full h-full scroll-area-2'):
                # Pass the single HTML blob
                # Add a 'chapter-2' class for our JS selector
                ui.html(chapter_html_2, sanitize=False).classes('p-4 chapter-2')
